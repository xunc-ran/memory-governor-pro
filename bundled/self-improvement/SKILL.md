...
> **Bundled in memory-lancedb-pro**: This tree lives at `memory-lancedb-pro/bundled/self-improvement/` in the same repository as the OpenClaw plugin. Runtime reminder injection + `.learnings` bootstrapping are integrated in the plugin; read `INTEGRATION.md` here for symlink/hook layout vs standalone installs.
...
**Already using memory-lancedb-pro:** You can symlink or copy this folder from the plugin repo, e.g. `…/memory-lancedb-pro/bundled/self-improvement` → `~/.openclaw/skills/self-improvement`, to use `scripts/` and this SKILL from the skills path without a separate clone.
