...
## Bundled copy (memory-lancedb-pro)

If you develop from the **memory-lancedb-pro** repository, this skill lives at `bundled/self-improvement/`. You can symlink that directory to `~/.openclaw/skills/self-improvement` instead of cloning another repo. The plugin already injects the LanceDB-backed reminder when `selfImprovement` is enabled — only install the **hook** from `hooks/openclaw` if you are not using that plugin path.
